...
def load_data_fromFile(inputFile):
...
if __name__ == "__main__":
	...
	inputFile = 'input_curve/test_caplet_1.csv'


	#-------------------------------------------------------

	mkt_prices_dict   = load_data_fromFile(inputFile)
	...
	ff      	= optimize.minimize(loss_caplet_model_vsck, x0_vsck, args=(mkt_prices_dict, tau), method = 'TNC',  bounds = x_vsck_bnd)
	...
	for tt in t_mkt_list:
		...
		mdl_tmp =caplet_price(list_model_params_opt, tt, tau)
	...
	plt.ylabel('Caplet prices')
	plt.xlabel('Caplet maturity')
