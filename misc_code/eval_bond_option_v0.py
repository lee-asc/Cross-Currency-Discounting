...
if __name__ == '__main__':
    ...
    vsck_discount_trj, vsck_2y_price_trj     = simulate_vsck_trj(r0, k, theta, sigma, n_trj, n_time_steps, p_mat)
    avg_forward_rate_mc, forward_rate_trj    = simulate_lmm_trj(delta_time_yy, maturity, zero_curve, forward_vols,n_trj)

    #print('avg_forward_rate_mc: ', avg_forward_rate_mc)
    ...
    for i in range(0, n_time_steps-1):
        ...
        if (i == 365*5):
            ...
            #print('i: ',i)
            discount_vsck   = vsck_discount_trj[i]
            p_zc_2y_vsck    = vsck_2y_price_trj[i]

            discount_lmm = compute_lmm_discount(n_trj, 5, forward_rate_trj)
            p_zc_2y_lmm  = compute_lmm_zc_price(5, 2, forward_rate_trj)


            p_tmp = p_zc_2y_lmm
            discount = discount_lmm
    ...
    price_mean  = np.mean(price)
    price_error = np.std(price)/np.sqrt(n_trj)

    print('Price: ',  np.round(price_mean, 4))
    print('P error: ', np.round(price_error, 4))
